...
calculate_metrics<-function(som_codes=NULL,som_pts=NULL,som_distances=NULL,wtdata=NULL,parallel=T,hex=F,log_file=NULL,verbose=F,standarize=T,stat=NULL){
...
create_som_model<-function(wtdata=NULL,model_params=NULL,parallel_mode=F,log_file=NULL,verbose=F,standarize=T){
  iam=match.call()[[1]]
  if(!exists("dependencyLoader")){
    if(!file.exists('functions_common/dependencyLoader.R')) return(list(error=TRUE,data=NULL,msg=paste0("\n",iam,":Missing dependency function: functions_common/dependencyLoader.R")));
    source('functions_common/dependencyLoader.R')
