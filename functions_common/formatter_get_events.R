formatter_get_events<-function(wt_query=NULL,
                               ...
                               array_id=NULL,
                               freq_dat_med_min=wt_query$freq_dat_med_min[1],
                               unix_timestamp_ini=wt_query$creation_wtdata_date_ini[1],
                               unix_timestamp_end=wt_query$creation_wtdata_date_end[1],
                               seconds_to_aggregate=wt_query$seconds_to_aggregate[1],
                               include_variables=wt_query$include_variables[1],
                               exclude_variables=wt_query$exclude_variables[1],
                               ...
                               db_config)
{
    ...
    if(nrow(columns)>0){
        ...
        rs <- db_query(query = query[1], db_config = db_config)
        if(rs$error) return(list(error=TRUE,data=NULL,msg=rs$msg))
